...
export const notifications = mysqlTable('notifications', {
  ...
  recipientId: varchar('recipient_id', { length: 128 }).notNull().references(() => users.id, { onDelete: 'cascade' }),
  senderId: varchar('sender_id', { length: 128 }).references(() => users.id, { onDelete: 'set null' }),
  
  type: mysqlEnum('type', [
    'comment', 'reply', 'mention', 'like', 'follow', 'bookmark',
    'post_published', 'post_featured', 'post_approved', 'post_rejected',
    'comment_approved', 'comment_rejected', 'account_verified',
    'welcome', 'newsletter', 'system_update', 'security_alert',
    'weekly_digest', 'trending_post', 'milestone_reached'
  ]).notNull(),
...
export const notificationSettings = mysqlTable('notification_settings', {
  ...
  bookmarkNotifications: boolean('bookmark_notifications').default(false),
  
  // Content notifications
  postPublishedNotifications: boolean('post_published_notifications').default(true),
  postFeaturedNotifications: boolean('post_featured_notifications').default(true),
  postModerationNotifications: boolean('post_moderation_notifications').default(true),
  
  // System notifications
  securityAlertNotifications: boolean('security_alert_notifications').default(true),
  systemUpdateNotifications: boolean('system_update_notifications').default(true),
  newsletterNotifications: boolean('newsletter_notifications').default(true),
  weeklyDigestNotifications: boolean('weekly_digest_notifications').default(true),
  ...
  // Frequency settings
  digestFrequency: mysqlEnum('digest_frequency', ['daily', 'weekly', 'monthly', 'never']).default('weekly'),
...
export const notificationQueue = mysqlTable('notification_queue', {
  id: varchar('id', { length: 128 }).primaryKey().$defaultFn(() => createId()),
  notificationId: varchar('notification_id', { length: 128 }).notNull().references(() => notifications.id, { onDelete: 'cascade' }),
